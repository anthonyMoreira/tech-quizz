...
test.describe('Quiz Play', () => {
  ...
  test('confirm button is disabled until an answer is selected', async ({ page }) => {
  ...
  test('progress updates through the quiz', async ({ page }) => {
    ...
    for (let q = 1; q <= 3; q++) {
      await expect(page.locator('.quiz-progress__numbers')).toHaveText(`${String(q)} / 10`);
      await answerCurrentQuestion(page);
      await goToNext(page);
    }
  ...
  test('completing all 10 questions navigates to results page', async ({ page }) => {
    await startQuiz(page, 'Domain-Driven Design');
